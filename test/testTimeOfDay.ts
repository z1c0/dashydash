...
import { TimeOfDayInfo } from '../src/backend/common/timeOfDayInfo'

function helper_SpecialTimeOfDay(day: number, month: number, hour: number) {
	var d = new Date(2016, month - 1, day);
	d.setHours(hour);
	return d;
}

function helper_SpecialDay(day: number, month: number) {
	return helper_SpecialTimeOfDay(day, month, 12);
}

function helper_TimeOfDay(hour: number) {
	var d = helper_WeekDay(2);
	d.setHours(hour);
	return d;
}

function helper_WeekDay(day: number) {
	return new Date(2016, 10, 20 + day);
}

function helper_TimeOfWeekDay(day: number, hour: number){
...
describe('timeOfDayInfo', function() {
		const tests = [
			...
			{ arg: helper_SpecialDay(22, 10), expected: 'CAPSLOCK' },
			...
			{ arg: helper_TimeOfDay(14), expected: 'Afternoon' },
			{ arg: helper_TimeOfDay(17), expected: 'Afternoon' },
			{ arg: helper_TimeOfDay(18), expected: 'Evening' },
			{ arg: helper_TimeOfDay(19), expected: 'BedTime' },
			{ arg: helper_TimeOfDay(23), expected: 'Evening' },
	...
	describe('getMatches()', function() {
		tests.forEach(function(test) {
			it(test.expected + ' matches at least once', function() {
				const matches = TimeOfDayInfo.getMatches(test.arg);
				assert.isAtLeast(matches.length, 1);
			});

			it(test.expected +  ' match has valid text property', function() {
				TimeOfDayInfo.getMatches(test.arg).forEach(m => {
	...
	describe('get()', function() {
		tests.forEach(function(test) {
			it(test.expected + ' matches to single result', function() {
				...
				var result = TimeOfDayInfo.get(test.arg, testRandomFunc);
				assert.isOk(result);
	...
	describe('logicMatcher', function() {
		var m = TimeOfDayInfo.matcher;
		it('evaluate single condition', function() {
			var e = m.is(m.specialDay(10, 3, false));
