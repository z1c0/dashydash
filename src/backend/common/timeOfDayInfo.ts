...
type DatePredicate = (_dt: Date) => boolean;
type MatchFactorReturnType = {
	_f: DatePredicate[]
	evaluate: DatePredicate
}
function matchFactor(f: DatePredicate) {
	return {
		_f : [ f ],
		evaluate : function(dt: Date) {
...
type MatchTermReturnType = {
	_f: MatchFactorReturnType[];
	evaluate: DatePredicate;
	or: (_f: DatePredicate) => MatchTermReturnType;
	and: (_f: DatePredicate) => MatchTermReturnType;
}
function matchTerm(f: DatePredicate) {
	return {
		_f : [ matchFactor(f) ],
		evaluate : function(dt: Date) {
			for (var i = 0; i < this._f.length; i++) {
		...
		or : function(f: DatePredicate) {
			this._f.push(matchFactor(f));
			return this;
		},
		and : function(f: DatePredicate) {
...
function timeOfDay(hourFrom: number, hourTo: number) {
	return function(dt: Date) {
		return dt.getHours() >= hourFrom && dt.getHours() < hourTo; 
	}
}

function specialDay(day: number, month: number, verySpecial: boolean) {
	return function(dt: Date) {
...
function timeOfWeekDay(day: number, hourFrom: number, hourTo: number) {
	return function(dt: Date) {
		return dayOfWeek(day)(dt) && timeOfDay(hourFrom, hourTo)(dt);
	}
}

function weekEnd(dt: Date) {
	return dayOfWeek(Day.Sa)(dt) || dayOfWeek(Day.So)(dt);
}

function birthday(who: string, day: number, month: number) {
	return {
		id : who,
...
type TextType = (string | [string, string[]]);
type Candidate = {
	id: string;
	match: MatchTermReturnType;
...
const candidates: Candidate[] = [
	...
	{
		id : 'CAPSLOCK',
		match : is(specialDay(22, 10, false)),
		probability : Probability.possible,
		text : [
			"CAPS LOCK DAY!!",
			"IT'S CAPS LOCK DAY!!"
		],
		tag : [ 'capslock', 'keyboard' ],
		emoji : [ 'keyboard' ]
	},
	{
		id : 'StarWars',
...
function getMatchesForTime(dt: Date): Match[] {
	let matches: Match[] = [];
...
function getSingleMatchForTime(dt?: Date, fRandom?: () => number) {
	if (!dt) {
		dt = new Date();
	}
	if (!fRandom) {
